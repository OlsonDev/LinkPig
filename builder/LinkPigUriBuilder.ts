...
export class LinkPigUriBuilder {
  ...
  open(pathOrParams: string | OpenParameters): this {
    const params = typeof pathOrParams === 'string' ? new OpenParameters({ relativePath: pathOrParams }) : pathOrParams;
  ...
  select(lineOrParams: number | SelectParameters, column?: number): this {
    const params =
      typeof lineOrParams === 'number' ? new SelectParameters({ line: lineOrParams, column }) : lineOrParams;
